...
| `officecli-local-review` | [`officecli-local-review.md`](./officecli-local-review.md) | User explicitly asks for `officecli-local-review`, local final PPTX review, or a post-export second opinion | Exported PPTX exists and passed Fusion structural/editability checks | Never replaces generation, export, or native PowerPoint QA | After Phase 6 export, before handoff | Advisory JSON and local PDF/PNG evidence | Local tools only; failure is advisory unless strict review was requested |
