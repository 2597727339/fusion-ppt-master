...
if (!file) {
  console.error('Usage: node scripts/validate-guizang-swiss-deck.mjs <index.html> [--allow-experimental]');
  process.exit(2);
}
...
slides.forEach((slide) => {
  ...
  if (!layout) {
    ...
    errors.push(`Slide ${slide.idx}: data-layout="${layout}" is not registered in references/guizang-swiss-layout-lock.md.`);
  }
