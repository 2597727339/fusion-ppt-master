...
class SVGQualityChecker:
    ...
    def check_file(self, svg_file: str, expected_format: str = None) -> Dict:
        ...
        try:
            ...
            # Comments are non-rendered metadata. Exclude them from regex-based
            # policy checks so documentation such as "No <style> or rgba()"
            # cannot be mistaken for live SVG markup.
            ...
            if self._check_xml_well_formed(content, result):
                # 1. Check viewBox
                self._check_viewbox(analysis_content, result, expected_format)

                # 2. Check forbidden elements
                self._check_forbidden_elements(analysis_content, result)

                # 3. Check fonts
                self._check_fonts(analysis_content, result)

                # 4. Check width/height consistency with viewBox
                self._check_dimensions(analysis_content, result)

                # 5. Check text wrapping methods
                self._check_text_elements(analysis_content, result)

                # 6. Check image references (file existence and resolution)
                self._check_image_references(analysis_content, svg_path, result)

                # 7. Check object-level animation anchor quality.
                self._check_animation_group_ids(analysis_content, result)

                # 7b. Check <pattern> elements declare a PPTX preset.
                self._check_pattern_fills(analysis_content, result)

                # 8. Check spec_lock drift (colors / font-family / font-size).
                #    Templates do not ship a spec_lock.md, so skip in template
                #    mode to avoid noise.
                if not self.template_mode:
                    self._check_spec_lock_drift(analysis_content, svg_path, result)

                # 9. Check web-sourced image attribution. Templates don't carry
                #    image_sources.json; skip in template mode.
                if not self.template_mode:
                    self._check_sourced_image_attribution(analysis_content, svg_path, result)
    ...
    def _check_spec_lock_drift(self, content: str, svg_path: Path, result: Dict):
        ...
        for m in re.finditer(
            r'font-family\s*=\s*(["\'])(.*?)\1', content, re.IGNORECASE
        ):
            val = m.group(2).strip()
    ...
    def _extract_spec_roster(spec_text: str) -> List[str]:
        ...
        for match in svg_ref_re.finditer(text):
            ...
            if stem in seen or not re.match(r"^(?:\d|S\d)", stem, re.IGNORECASE):
                continue
            seen.add(stem)
            stems.append(stem)
