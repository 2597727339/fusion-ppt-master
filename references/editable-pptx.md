...
浏览器里刚好放下，不代表 PowerPoint 里也安全。`html2pptx.js` 导出的真文本框，在 PowerPoint 中经常比浏览器测得更宽一点；卡片里那种“差一点点就碰边”的文案，导出后最容易炸。

做卡片、说明框、标签、按钮文案时，默认遵守下面几条：

1. 宁可早换行，不要赌单行刚好装下。
2. 窄卡片正文只占 `70%-80%` 可用宽度，右边必须留真正的缓冲区。
3. 圆角容器比直角容器更保守，视觉可用空间会被右上和右下圆角吃掉。
4. 标题、正文、标签并存时，优先加高容器，不要为了省一点高度把正文挤成危险单行。
5. 导出后必须做一次 PowerPoint 真机检查；看到贴边、出框、被裁掉，修源文件，不接受“HTML 里明明正常”。

### 保守排版建议

| 场景 | 建议 |
|---|---|
| `<= 260pt` 宽的说明卡片 | 正文默认拆成 2-3 行 |
| 深色卡片里的浅色正文 | 字号比浅底卡再小 `1pt`，行高再大一点 |
| 一行里同时有中文 + 英文 / 路径 / 斜杠 | 更早换行，别把 `/` 这种连接串放到右侧边缘 |
| pill / badge / tag | 文案只放短词，不塞整句 |
