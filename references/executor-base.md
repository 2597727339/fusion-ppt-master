...
### 2.0 Expanded Layout Library

The fusion skill ships with **three layout families** beyond the base templates:

| Family | Directory | Count | Origin | Style |
|--------|-----------|-------|--------|-------|
| Base | `templates/layouts/*/` | varies | ppt-master original | varies per template |
| Magazine Style A | `templates/layouts/magazine-style-a/` | 12 | `references/guizang-layouts.md` | Editorial, narrative rhythm, hero covers, big quotes, data grids |
| Swiss International PPTX | `templates/layouts/swiss-international/` | 22 | local `design_spec.md` | Typographic hierarchy, strict grids, hairline rules, minimal decoration |

When Strategist assigns a `page_layouts` entry referencing a magazine or Swiss basename, the Executor inherits the corresponding SVG from the family directory. The `layouts_index.json` maps each basename to its family. Swiss HTML uses a separate locked `data-layout="Sxx"` registry; never infer PPTX SVG semantics from `references/guizang-swiss-layout-lock.md`, and never infer HTML semantics from the SVG basename.
...
- **Tight-card safety recipe**: when prose lives inside a rounded card, callout, KPI box, or label panel, wrap earlier than the mathematical maximum. Target roughly `70-80%` usable line occupancy, because exported PowerPoint text commonly measures wider than SVG preview text. If a sentence is close to the right edge, add a line break or increase the card height before export.
