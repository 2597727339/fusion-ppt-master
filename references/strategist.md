...
### b. Page Count Confirmation (Guizang-enhanced)

Provide specific page count recommendation based on:
- Source document content volume
- **Presentation duration** (if provided): ~2 minutes per page average
- **Narrative shape**: investment-case typically 10-15 pages; teaching-narrative 15-30 pages
- **Guizang rhythm constraints**: max 3 consecutive dense pages per topic; breathing pages reduce effective content capacity

### c. Key Information Confirmation (Enhanced with Dashiai)

> **Source**: [`dashiai-narrative-shapes.md`](dashiai-narrative-shapes.md)

Confirm target audience, usage occasion, and core message. Additionally:

#### Audience-First Framework (Dashiai)

Before recommending content structure, answer:

1. **Who makes the decision?** — role, seniority, time pressure
2. **What is their current belief?** — aligned, skeptical, unaware
3. **What would make them change their mind?** — evidence type they trust
4. **What are likely objections?** — top 3 counterarguments

#### Narrative Shape Selection (Dashiai)

Choose ONE narrative arc from [`dashiai-narrative-shapes.md`](dashiai-narrative-shapes.md) §2:

| Shape | One-line thesis pattern |
|-------|------------------------|
| problem-to-solution | "If we don't solve ___, ___ happens; if we do ___, ___ improves" |
| strategic-recommendation | "Based on ___, we recommend ___ because it optimizes ___" |
| investment-case | "This is a ___ market opportunity with ___ moat, seeking ___ round" |
| operating-review | "This period we achieved ___, ___ above/below expectation because ___" |
| proposal | "We understand your need for ___, our solution delivers ___ for ___" |
| launch-story | "We discovered users need ___, so we created ___ and the market responded ___" |
| teaching-narrative | "After learning, you will be able to ___. Let's understand via ___" |

**Decision tree**:
```
Does the deck require a decision?
  ├── Yes, choose direction → strategic-recommendation
  ├── Yes, invest money → investment-case
  ├── Yes, approve project → proposal
  └── No, mainly communicate/educate
      ├── Change mind → problem-to-solution
      ├── Review period → operating-review
      ├── Release something → launch-story
      └── Teach knowledge → teaching-narrative
```

#### Thesis Statement

Write a ONE-sentence thesis (not a topic list):
- Must be a **claim**, not a topic
- Must be **arguable** — someone could disagree
- Max 25 Chinese characters or 20 English words

#### Evidence Gaps (Dashiai)

Classify every major claim:

| Status | Meaning | Action |
|--------|---------|--------|
| verified | Supported by data/facts | Display normally |
| assumption | Reasonable inference, no direct data | Mark as assumption, suggest validation |
| missing | Evidence needed | Flag in speaker notes as risk |

### d. Style Objective Confirmation (Enhanced with Huashu)

> **Source**: [`design-styles-index.json`](design-styles-index.json) + [`huashu-design-direction.md`](huashu-design-direction.md) + [`huashu-taste-anchors.md`](huashu-taste-anchors.md) + [`huashu-anti-slop.md`](huashu-anti-slop.md)
...
#### Layer 2 — Visual style (Huashu-enhanced)

Anchors the downstream confirmations e (Color), f (Icon), g (Typography), h (Image).

**When user names a style** → record verbatim as a short descriptor (normalize multilingual phrasings to a single canonical form).

**When user does NOT name a style** → activate **Design Direction Consultant Mode** (huashu):

1. Analyze source document for 3 features: industry/domain, content density, emotional tone
2. Propose **3 differentiated directions** from [`design-styles-index.json`](design-styles-index.json):
   - **Direction A (conservative)**: industry convention, zero risk
   - **Direction B (moderate)**: slight novelty, controlled risk
   - **Direction C (bold)**: unconventional, needs persuasion
3. Present each direction in this format:
   ```
   [Plan A] <style name> — <one-line description>
     Visual: font pairing / color倾向 / layout character
     Suitable: scenarios and audience
     Reference: real-world analogy (company/publication/event)
   ```
4. After user picks → e/f/g/h auto-align to the chosen style

**Style recall from [`design-styles-index.json`](design-styles-index.json)**:

- 20 PPT styles + 20 web styles (adapted for presentation use)
- Each style has: temperature (bold/moderate/calm), industry_fit, palette, typography, icon_style, rendering, palette_rec
- **Temperature balance**: model's certainty bias defaults to calm/minimal — the库 intentionally has 8 bold PPT styles to counteract this
...
**Taste Anchors** (positive guardrails — what we WANT, not what we don't):

| Anchor | Example | Effect |
|--------|---------|--------|
| Typography | "serif display + sans body" | editorial feel |
| Color | "single warm color throughout" | restraint, focus |
| Density | "information density typewriter: cover breathing, body dense" | rhythm |
| Material | "paper texture at 8% opacity" | tactile feel |

**Anti-slop enforcement** (from [`huashu-anti-slop.md`](huashu-anti-slop.md)):

The following are forbidden as default AI styling. Purple gradients and rounded-card systems may be used only when an explicit brand rule or user-approved named style requires them, with the rationale and boundary recorded in `spec_lock.md visual_tone`. Emoji icons, SVG-drawn people silhouettes, and uniform page density remain forbidden shortcuts.
...
|--------|----------|-------------------|
| **A** | Built-in SVG icon library | Professional decks; recommended default |
| **B** | User-provided brand or custom icons | Verified brand assets already exist |
| **C** | No icons | Editorial, typographic, or data-first decks where icons add no meaning |

Do not offer emoji or AI-generated raster symbols as icon options. AI-generated illustrations belong to image confirmation `h`, not icon confirmation `f`.
