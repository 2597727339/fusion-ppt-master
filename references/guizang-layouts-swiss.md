...
本主题的本地 golden source 是 `templates/html/guizang/template-swiss.html`、`references/guizang-swiss-layout-lock.md` 和 `templates/layouts/swiss-international/`。不得依赖原作者机器上的外部路径。

生成正文页时不要把 Swiss 当成“自由组合的风格包”。默认只能使用 `references/guizang-swiss-layout-lock.md` 登记的 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。
...
- 地点、路线、人物住所、城市关系页使用 `S08 + Swiss Map Component`;这仍然是 S08 的右侧插槽扩展,不是新正文页。先读 `references/guizang-swiss-map-component.md`。
- SVG 只画几何,不写可见文字。标签放 HTML 里。
- 生成完成后运行 `node scripts/validate-guizang-swiss-deck.mjs index.html`。

---

## 设计语言基线

**配色**(`--accent` 由主题决定,见 `references/guizang-themes-swiss.md`)
...
1. 同时打开本地 `templates/html/guizang/template-swiss.html`、当前生成页和正在修改的测试页。若用户另行提供原始参考 PPT,再把它加入对照;不得假设外部 golden source 存在。
