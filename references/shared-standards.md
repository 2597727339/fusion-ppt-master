...
### Text Fit Safety (Mandatory for PPT Export)

PowerPoint's native text layout is more conservative than browser or SVG preview text metrics. A line that "just fits" in SVG often renders wider in PPTX and spills across rounded-card edges.

Treat every card, callout, pill, and side-note as needing a safety margin:

- Never pack copy to the visual edge. Text should stop well before the right border.
- Reserve extra interior width for narrow containers: keep at least `24-32px` of effective spare room on the right side after wrapping.
- Safety is bilateral, not one-sided. A line is **not** acceptable when the left inset looks roomy but the right edge nearly kisses the border; treat that as overflow pressure and reflow it.
- Keep padding visually balanced. In cards, pills, dark callouts, and rounded labels, the rendered right-side spare room should feel close to the left inset rather than collapsing to a thin strip.
- Prefer one more line over one tighter line. In CJK decks, if a sentence sits inside a box narrower than roughly `360px`, proactively split it into `2-3` manual lines with sibling `<text>` elements or `<tspan x="..." dy="...">`.
- For centered pill / badge copy, shorten lines early and increase container width or height before shrinking type. Rounded corners visually reduce usable width more than rectangular math suggests.
- Rounded cards need more slack than square cards. The right-side corner curves visually eat into usable space.
- If a card contains a heading plus multiple body lines, grow the card before shrinking the text aggressively.
- If a PPT render shows "technically inside the box" but optically pressed against the right edge, that still fails review. Re-break the line or enlarge the box until both sides breathe.
- Text may not run underneath a neighboring floating card, mock window, or highlight panel. Cross-block collision counts as overflow too; reflow one block or move the adjacent panel.
- Post-export reality wins. If PowerPoint render shows edge contact or overflow, fix the source SVG layout instead of accepting the preview.
