...
本地参考基线由 `templates/html/guizang/template-swiss.html`、本文件和 `templates/layouts/swiss-international/` 共同组成。不得读取或假设原作者机器上的外部文件。
...
2. 不允许临时发明 `P23/P24` 这类未出现在原始 22P 的正文结构。需要图片时,优先使用 `S22 Image Hero`;多图时使用 `S15/S16` 的原始网格骨架做图片格改造,不要发明新的证据墙。唯一登记的交互扩展是 `S08 + Swiss Map Component`,详见 `references/guizang-swiss-map-component.md`。
...
- 详细代码和数据契约见 `references/guizang-swiss-map-component.md`。
...
- 如果原始截图比例不可控,先按 `references/guizang-screenshot-framing.md` 做程序化比例适配;只有长截图、极窄截图或信息需要重构时,才用图像生成器重生成“截图再设计”。
