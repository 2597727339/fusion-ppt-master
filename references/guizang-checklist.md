...
**做法**:
- 先读 `references/guizang-swiss-layout-lock.md`
- 正文页只能使用 `S01-S22`;新增首页/尾页只能使用 `SWISS-COVER-ASCII` / `SWISS-CLOSING-ASCII`
- 每个 `<section class="slide">` 必须写 `data-layout="Sxx"`
- 生成后必须运行:

```bash
node <skill_root>/scripts/validate-guizang-swiss-deck.mjs path/to/index.html
```
...
**做法**:
- 使用 `data-layout="S08"`。
- 先读 `references/guizang-swiss-map-component.md`。
...
- 用户原始截图要先读 `references/guizang-screenshot-framing.md`:优先用 `templates/html/guizang/assets/screenshot-backgrounds/` 内置主题背景 + 程序化缩放/留边/对齐,不要为了比例统一就重画截图内容
...
**做法**:
- 本地 `templates/html/guizang/template-swiss.html`、Swiss layout lock 和 22 个登记 SVG 是可移植的 golden source;若用户提供额外参考,再加入对照
...
- 如果要修改 `templates/html/guizang/template-swiss.html`,先做本地基线对比;可接受差异只应是 ASCII 类、S22 图片定位类、轻字重标题 helper 和已知动效修复
...
**做法**：
- **生成 HTML 前,必须先 `Read` 当前风格对应模板**:风格 A 读 `templates/html/guizang/template.html`,风格 B 读 `templates/html/guizang/template-swiss.html`,确认 layouts 里用到的类都已定义
...
动效
  □ `templates/html/guizang/assets/motion.min.js` 已复制为输出目录的 `assets/motion.min.js`(本地兜底)
