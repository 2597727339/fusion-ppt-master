...
|---|---|
| Git clone | Run `<python> scripts/update_repo.py` inside the `ppt-master` folder |
...
```bash
<python> scripts/svg_to_pptx.py <project_path> --no-merge
```
...
Two independent choices are locked during the Eight Confirmations:

- **Narrative shape** (how the deck supports a decision): select one primary arc from `references/dashiai-narrative-shapes.md`.
- **Visual direction** (how it looks): select from the 40-direction catalog in `references/design-styles.md`, or record a custom direction.

The narrative and visual choices may be paired freely when the content remains legible and evidence-led.
...
```bash
<python> scripts/svg_to_pptx.py <project> -t push       # different transition
<python> scripts/svg_to_pptx.py <project> -t none       # disable transitions
<python> scripts/svg_to_pptx.py <project> -a auto       # enable per-element entrance (effect mapped from group id)
<python> scripts/svg_to_pptx.py <project> --animation fade        # enable with a single effect
<python> scripts/svg_to_pptx.py <project> -a auto --animation-trigger on-click   # presenter-paced reveals
```
...
Use **beautify** when the source deck's page split is part of the requested output: text stays verbatim, page count and order are preserved 1:1, only layout / hierarchy / whitespace are redone while inheriting the original palette/fonts. Say "make this deck look better" / "re-layout this, keep the wording". See the [beautify workflow](../workflows/beautify-pptx.md).
...
> For more questions, see [SKILL.md](../SKILL.md).
