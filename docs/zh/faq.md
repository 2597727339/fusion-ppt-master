...
| 安装方式 | 更新方式 |
|---|---|
| Git clone | 在 `ppt-master` 目录运行 `<python> scripts/update_repo.py` |
...
- **主叙事弧（怎么支持决策）**：从 `references/dashiai-narrative-shapes.md` 的七种主弧中选择一个。
- **视觉方向（长什么样）**：从 `references/design-styles.md` 的 40 个方向中选择，或明确记录自定义方向。

只要内容保持清晰且证据优先，叙事与视觉可以自由组合。
...
```bash
<python> scripts/svg_to_pptx.py <project> -t push       # 换转场效果
<python> scripts/svg_to_pptx.py <project> -t none       # 关闭转场
<python> scripts/svg_to_pptx.py <project> -a auto       # 开启页内元素入场（按 group id 自动映射效果）
<python> scripts/svg_to_pptx.py <project> --animation fade        # 开启并改用单一效果
<python> scripts/svg_to_pptx.py <project> -a auto --animation-trigger on-click   # 单击触发，演讲者控制节奏
```
...
用 **beautify** 的前提是：原 PPT 的分页本身就是输出要求的一部分。文字逐字不动、页数页序 1:1 保留，只重排版式、层级和留白，并继承原配色字体。典型说法是「把这份 PPT 美化一下 / 重新排版，内容别动」。见 [beautify 工作流](../../workflows/beautify-pptx.md)。
...
输出仍是 100% 原生可编辑的 PowerPoint：原设计、母版、图片、动画都保留，且只导出选中的页面。它刻意**不**改版式、不加页、不换图——一份 deck 的页面结构本身承载着逻辑（总分、对比、递进），所以应挑选结构本就契合内容的页面，而不是硬塞进去。若需要全新结构或不同页数，请改用 create-template（见下一问）。完整步骤：[套模板工作流](../../workflows/template-fill-pptx.md)。
...
> 更多问题可先查看 [SKILL.md](../../SKILL.md) 与 [SKILL.md](../../SKILL.md)
