...
Executable route selection is authoritative in [`workflows/routing.md`](../workflows/routing.md); this section is a rationale-oriented quick reference, not a second route matrix to maintain.
...
Artifact source/derived ownership is authoritative in [`artifact-ownership.md`](../references/artifact-ownership.md); this section visualizes the same dataflow for architecture rationale.
...
Executable artifact ownership invariants are authoritative in [`artifact-ownership.md`](../references/artifact-ownership.md); this section explains why those boundaries matter architecturally.
...
This is the key insight that's easy to miss when reading the code. The same modules under `scripts/svg_finalize/` are used in two places, for two different products.
...
The standalone workflow registry is authoritative in [`workflows/index.md`](../workflows/index.md); this section explains why these capabilities stay separate.
